...
on conflict (base44_id) do update set
    nome = excluded.nome,
    email = case
        when excluded.email not like 'sem-email-%@garagehub.local' then excluded.email
        when public.usuarios.email like 'sem-email-%@garagehub.local' then excluded.email
        else public.usuarios.email
    end,
