...
-- Garantias para rodar também em tabelas já existentes.
alter table public.usuarios add column if not exists base44_id text;
alter table public.usuarios add column if not exists nivel_cliente text default 'comum';
...
alter table public.usuarios add column if not exists codigo_membro text;
alter table public.usuarios add column if not exists criado_em timestamptz default now();

create unique index if not exists idx_usuarios_base44_id_unique
on public.usuarios(base44_id)
...
-- Correção: evita e-mail nulo e evita conflito de e-mail duplicado.
with clientes_preparados as (
    select
        c.*,
        lower(nullif(trim(c.email), '')) as email_original,
        row_number() over (
            partition by lower(nullif(trim(c.email), ''))
            order by c.base44_id
        ) as ordem_email
...
clientes_final as (
    select
        ...
        case
            when c.email_original is null then
                'sem-email-' || c.base44_id || '@garagehub.local'

            when c.ordem_email > 1 then
                'sem-email-' || c.base44_id || '@garagehub.local'

            when exists (
                select 1
                from public.usuarios u2
                where lower(u2.email) = c.email_original
                  and u2.base44_id is distinct from c.base44_id
            ) then
                'sem-email-' || c.base44_id || '@garagehub.local'
        ...
        c.estado,
        c.foto_url,
        coalesce(c.created_date, now()) as criado_em,
        concat('GHW-', coalesce(c.id::text, c.base44_id)) as codigo_membro
    from clientes_preparados c
)
...
on conflict (base44_id) do update set
    ...
    email = case
        when exists (
            select 1
            from public.usuarios u2
            where lower(u2.email) = lower(excluded.email)
              and u2.base44_id is distinct from excluded.base44_id
        ) then public.usuarios.email
        else excluded.email
    end,
